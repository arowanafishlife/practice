...
# goodボタンの処理
@login_required(login_url='/admin/login/')
def good(request, good_id, before, user_id=''):
    # goodするMessageを取得
    ...
    if is_good > 0:
        ...
        return redirect(to='/sns/'+ str(before) + '/' + user_id)
...
def followed(request, target_id):
    ...
    for f in fled:
        ...
        if s.count() > 0:
            ...
            create_default_profile(f.owner)
...
# delete_messageのビュー関数
@login_required(login_url='/admin/login/')
def delete_message(request, message_id, before, user_id=''):
